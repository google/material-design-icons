## Material Symbols / Material Icons

These are two different official icon sets from Google, using the same underlying designs. Material Symbols is the current set, introduced in April 2022, built on variable font technology. Material Icons is the classic set, but no longer updated. More details below.

The icons can be browsed in a more user-friendly way at https://fonts.google.com/icons. Use the popdown menu near top left to choose between the two sets; Material Symbols is the default.

The icons are designed under the [material design guidelines](https://material.io/guidelines/).

## Icon Requests

We’d love to support your icon needs! Please submit your request here on GitHub as an issue.

Please note that Google Fonts does not accept user submissions of finished icon designs! There are fairly strict  guidelines for Material icons, plus Google has upstream source files from which this repo is generated. Therefore, Google does not accept pull requests for icon files (whether new icon suggestions, or fixes for existing icons). Concepts are appreciated—just don’t design SVGs and submit them via pull request.

However, users are perfectly welcome to point at outside files or images as examples—for the kind of thing they want, but they won’t just be taken “as is.” This works especially well if you have multiple examples for a single icon, to help us understand the “essence” of the idea.

> For example, there is a fairly universal conceptual logo/icon for “agender,” so if you were proposing Google add an agender icon in the Material style, either mentioning that, or pointing at https://www.google.com/search?q=agender+icon would be a helpful tip.

### Third-party logos

Currently, Google does not include 3rd-party logos among the Material Symbols or Material Icons due to legal reasons. Some 3rd-party logos that were included in the past have since been removed.

## npm Packages

Google does not currently maintain the npm package for this repo, past v3 (2016). However, user @marella is hosting the following. He tells us these are automatically updated and published using GitHub Actions. Note: Google does **not** monitor or vet these packages.

### [material-symbols](https://github.com/marella/material-symbols/tree/main/material-symbols#readme) [![npm](https://img.shields.io/npm/v/material-symbols)](https://www.npmjs.com/package/material-symbols) [![install size](https://packagephobia.com/badge?p=material-symbols)](https://packagephobia.com/result?p=material-symbols)

- Only WOFF2 variable fonts and CSS for Material Symbols
- Includes outlined, rounded, and sharp icons and all variations of fill, weight, grade, and optical size
- Supports Sass

### [material-icons](https://github.com/marella/material-icons#readme) [![npm](https://img.shields.io/npm/v/material-icons)](https://www.npmjs.com/package/material-icons) [![install size](https://packagephobia.com/badge?p=material-icons)](https://packagephobia.com/result?p=material-icons) [![Downloads](https://img.shields.io/npm/dm/material-icons)](https://www.npmjs.com/package/material-icons)

- Only WOFF2, WOFF fonts and CSS
- Includes outlined, round, sharp and two-tone icons
- Supports Sass

### [@material-design-icons/font](https://github.com/marella/material-design-icons/tree/main/font#readme) [![npm (scoped)](https://img.shields.io/npm/v/@material-design-icons/font)](https://www.npmjs.com/package/@material-design-icons/font) [![install size](https://packagephobia.com/badge?p=@material-design-icons/font)](https://packagephobia.com/result?p=@material-design-icons/font)

- Only WOFF2 fonts and CSS
- Lighter version of `material-icons` package
- Doesn't support [older browsers](https://caniuse.com/woff2) such as Internet Explorer because of dropping WOFF (v1)

### [@material-design-icons/svg](https://github.com/marella/material-design-icons/tree/main/svg#readme) [![npm (scoped)](https://img.shields.io/npm/v/@material-design-icons/svg)](https://www.npmjs.com/package/@material-design-icons/svg) [![install size](https://packagephobia.com/badge?p=@material-design-icons/svg)](https://packagephobia.com/result?p=@material-design-icons/svg)

- Only SVGs
- Optimizes SVGs using SVGO

## Material Symbols

These newer icons can be browsed in a more user-friendly way at https://fonts.google.com/icons. Use the popdown menu near top left to choose between the two sets; Material Symbols is the default.

These icons were built/designed as variable fonts first (based on the 24 px designs from Material Icons). There are three separate Material Symbols variable fonts, which also have static icons available (but those do not have all the variations available, as that would be hundreds of styles):
- Outlined
- Rounded
- Sharp
- Note that although there is no separate Filled font, the Fill axis allows access to filled styles, in all three fonts. It can also be manipulated for an animated fill effect, to indicate user selection.

Each of the fonts has these design axes, which can be varied in CSS, or in many more modern design apps:
- Optical Size (opsz) from 20 to 48 px. The default is 24.
- Weight from 100 (Thin) to 700 (Bold). Regular is 400.
- Grade from -50 to 200. The default is 0 (zero). -50 is suggested for reversed contrast (e.g. white icons on black background)
- Fill from 0 to 100. The default is 0 (zero).

The following directories in this repo contain specifically Material Symbols (not Material Icons) content:
- symbols
- variablefont

What is currently _not_ available in Material Symbols?
- only the 20 and 24 px versions are designed with perfect pixel-grid alignment
- the only pre-made fonts are the variable fonts
- there are no two-tone icons

## Material Icons

The icons can be browsed in a more user-friendly way at https://fonts.google.com/icons?icon.set=Material+Icons

These classic icons are available in five distinct styles:
- Outlined
- Filled (the font version is just called Material Icons, as this is the oldest style)
- Rounded
- Sharp
- Two tone

The following directories in this repo contain specifically Material Icons (not Material Symbols) content:
- android
- font
- ios
- png
- src

What is currently _not_ available in Material Icons?
- variable fonts
- weights other than Regular
- grades other than Regular
- a means to animate Fill transitions
- new icons (since updates were halted in 2022)

## Material Icons update history

### 4.0.0 Update
* 2020 Aug 31
* Restructured repository, updated assets.

### 3.0.1 Update
* 2016 Sep 01
* Changed license in package.json.
* Added missing device symbol sprites.

### 3.0.0 Update
* 2016 Aug 25
* License change to Apache 2.0!

### 2.0
* 2016 May 28

## Getting Started

Read the [developer guide](https://developers.google.com/fonts/docs/material_icons) on how to use the material design icons in your project.

### Using a font

The `font` and `variablefont` folders contain pre-generated font files that can be included in a project. This is especially convenient for the web; however, it is generally better to link to the web font hosted on Google Fonts:

```html
<link href="https://fonts.googleapis.com/css2?family=Material+Icons"
      rel="stylesheet">
```

```html
<link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined"
      rel="stylesheet">
```
Read more on [Material Symbols](https://developers.google.com/fonts/docs/material_symbols/) or [Material Icons](https://developers.google.com/fonts/docs/material_icons/) in the Google Fonts developer guide.


## License

We have made these icons available for you to incorporate into your products under the [Apache License Version 2.0](https://www.apache.org/licenses/LICENSE-2.0.txt). Feel free to remix and re-share these icons and documentation in your products.
We'd love attribution in your app's *about* screen, but it's not required.


## 🌐 Web Resources & Interactive Index
- [HALLOWEEN FRUIT SLICE](https://studyplaying.github.io/halloween-fruit-slice.html)
- [INDEX2](https://quizverses.github.io/index2.html)
- [BOLTS UNSCREW IT](https://theskillquest.pages.dev/bolts-unscrew-it.html)
- [INDEX2](https://studyquests.github.io/index2.html)
- [CUBICA](https://themindzone.pages.dev/cubica.html)
- [CATEGORY THINKY](https://quizverses.pages.dev/category-thinky.html)
- [SLITHERCRAFT IO](https://studyplayings.pages.dev/slithercraft-io.html)
- [AQUA SORT WATER COLOR PUZZLE](https://iskillquest.pages.dev/aqua-sort-water-color-puzzle.html)
- [ASOKA MAKEUP INDIAN BRIDE](https://iskillquest.pages.dev/asoka-makeup-indian-bride.html)
- [EAT DONUTS](https://iskillquest.pages.dev/eat-donuts.html)
- [CATEGORY FLASH](https://themindzone.pages.dev/category-flash.html)
- [STICKMAN SHOOTER BROS](https://studyplayings.web.app/stickman-shooter-bros.html)
- [POTTERY MASTER](https://themindplays.pages.dev/pottery-master.html)
- [POP THEM](https://thelearnquester.web.app/pop-them.html)
- [FLAG PUZZLE JAM COLLECT FLAGS](https://iskillquest.pages.dev/flag-puzzle-jam-collect-flags.html)
- [TIKTOK TRENDS COLORED DENIM](https://thelearnquester.web.app/tiktok-trends-colored-denim.html)
- [LABUBA MERGE](https://quizverses.github.io/labuba-merge.html)
- [K POP HUNTER FASHION](https://quizverses.github.io/k-pop-hunter-fashion.html)
- [CATEGORY LOVE12](https://studyplayings.pages.dev/category-love12.html)
- [STICK KILL 3D](https://theskillquest.pages.dev/stick-kill-3d.html)
- [SEA MONSTERS MAHJONG](https://learnquesters.pages.dev/sea-monsters-mahjong.html)
- [SCREW PUZZLE MASTER](https://thelearnquester.web.app/screw-puzzle-master.html)
- [CONNECT 2 CARS](https://studyplayings.web.app/connect-2-cars.html)
- [GEOMETRY LITE](https://themindplay.pages.dev/geometry-lite.html)
- [CATEGORY UNBLOCKER](https://studyquests.github.io/category-unblocker.html)
- [PUZZLE WOOD BLOCK](https://thelearnquesters.pages.dev/puzzle-wood-block.html)
- [MR RACER CAR RACING](https://learnquesters.pages.dev/mr-racer-car-racing.html)
- [STUMBLE GUYS](https://quizverses-9d2f2.web.app/stumble-guys.html)
- [MUSHROOM FEVER MATCH 3](https://themindplays.pages.dev/mushroom-fever-match-3.html)
- [EXCAVATOR SIMULATOR 3D](https://thelearnquesters.pages.dev/excavator-simulator-3d.html)
- [CATEGORY SOCCER](https://quizverses.github.io/category-soccer.html)
- [OVERFLOWING PALETTE](https://quizverses-9d2f2.web.app/overflowing-palette.html)
- [CATEGORY 3 PLAYER26](https://themindplay.github.io/category-3-player26.html)
- [ULTRA PIXEL SURVIVE 2](https://iskillquest.pages.dev/ultra-pixel-survive-2.html)
- [CATEGORY CASUAL969](https://thelearnquesters.pages.dev/category-casual969.html)
- [CATEGORY TOWER DEFENSE 3](https://themindskillplayplay.pages.dev/category-tower-defense-3.html)
- [COIN MERGE MACHINE](https://thequizzone.pages.dev/coin-merge-machine.html)
- [URBAN ASSAULT FORCE](https://quizverses-9d2f2.web.app/urban-assault-force.html)
- [CATEGORY BATTLE ROYALE25](https://skillplay.github.io/category-battle-royale25.html)
- [VOXIOM IO](https://quizverses.github.io/voxiom-io.html)
- [STRAWBERRY SHORTCAKE](https://studyplayings.web.app/strawberry-shortcake.html)
- [DIG OUT OF PRISON](https://skillplay.github.io/dig-out-of-prison.html)
- [CATEGORY STORY45](https://thelearnquester.web.app/category-story45.html)
- [SNAKE 2048](https://themindskillplayplay.pages.dev/snake-2048.html)
- [KNOCKOUT DUDES](https://quizverses.github.io/knockout-dudes.html)
- [DIY MAKEUP SALON SPA MAKEOVER STUDIO](https://thelearnquesters.pages.dev/diy-makeup-salon-spa-makeover-studio.html)
- [SITEMAP](https://brainquests.netlify.app/sitemap.html)
- [CATEGORY CAR 2](https://skillplay.github.io/category-car-2.html)
- [CATEGORY MOUSE1 697](https://themindplay.github.io/category-mouse1-697.html)
- [CATEGORY PREMIUM PERKS71](https://studyplayings.pages.dev/category-premium-perks71.html)
- [BLOCK PUZZLE FROZEN JEWEL](https://thelearnquester.web.app/block-puzzle-frozen-jewel.html)
- [IDLE TOWN BILLIONAIRE](https://thequizzone.pages.dev/idle-town-billionaire.html)
- [FUN MINI GAMES FOR KIDS](https://iskillquest.pages.dev/fun-mini-games-for-kids.html)
- [HOME RUSH THE FISH WAR](https://theskillquest.pages.dev/home-rush-the-fish-war.html)
- [GOOD YARD](https://themindplay.pages.dev/good-yard.html)
- [CRAZY GOOSE SIMULATOR](https://thelearnquesters.pages.dev/crazy-goose-simulator.html)
- [BOARD KINGS BOARD DICE](https://studyplayings.web.app/board-kings-board-dice.html)
- [2048 CUBE SHOOTING MERGE](https://themindplay.pages.dev/2048-cube-shooting-merge.html)
- [CATEGORY CASUAL 3](https://studyplayings.pages.dev/category-casual-3.html)
- [MERGE HERO SURVIVAL TOWER DEFENSE](https://quizverses-9d2f2.web.app/merge-hero-survival-tower-defense.html)
- [CUT N FILL](https://iskillplay.web.app/cut-n-fill.html)
- [ISOMETRIC ESCAPE](https://iskillquest.pages.dev/isometric-escape.html)
- [CATEGORY CASUAL 8](https://skillplay.github.io/category-casual-8.html)
- [LION FAMILY SIM ONLINE](https://theskillquest.pages.dev/lion-family-sim-online.html)
- [DIGITAL AQUA](https://thelearnquester.web.app/digital-aqua.html)
- [JIGSORT PUZZLES](https://thequizzone.pages.dev/jigsort-puzzles.html)
- [HALLOWEEN CHALLENGE](https://themindskillplayplay.pages.dev/halloween-challenge.html)
- [EVONY THE KINGS RETURN](https://studyplayings.web.app/evony-the-kings-return.html)
- [RUSSIAN CHECKERS](https://learnquesters.pages.dev/russian-checkers.html)
- [SUMMER MAZE](https://theskillquest.pages.dev/summer-maze.html)
- [MYCOTHREEMEMO](https://skillplay.github.io/mycothreememo.html)
- [DRAWING SQUARES](https://thelearnquesters.pages.dev/drawing-squares.html)
- [BACK TO SCHOOL UNIFORMS EDITION](https://quizverses.github.io/back-to-school-uniforms-edition.html)
- [GAL SLIDING PUZZLE](https://thequizzone.pages.dev/gal-sliding-puzzle.html)
- [BAD EGG](https://themindplay.github.io/bad-egg.html)
- [FRUIT MAHJONG 3D](https://quizverses-9d2f2.web.app/fruit-mahjong-3d.html)
- [PICTURE BY PIECES](https://themindplay.pages.dev/picture-by-pieces.html)
- [QUEEN OF MAHJONG](https://iskillplay.web.app/queen-of-mahjong.html)
- [HEAD SOCCER ARENA](https://skillplay.github.io/head-soccer-arena.html)
- [BLANKETS](https://themindplay.pages.dev/blankets.html)
- [DRAGON HUNTER](https://quizverses-9d2f2.web.app/dragon-hunter.html)
- [SKY ASSAULT](https://skillplay.github.io/sky-assault.html)
- [FORTRESS OF THE SINISTER](https://thequizzone.pages.dev/fortress-of-the-sinister.html)
- [PAPA BUZJA](https://thequizzone.pages.dev/papa-buzja.html)
- [WILD WEST MATCH](https://thequizzone.pages.dev/wild-west-match.html)
- [BEAM DRIVE CAR CRASH TEST SIMULATOR](https://iskillquest.pages.dev/beam-drive-car-crash-test-simulator.html)
- [CATEGORY ADVENTURE 2](https://thelearnquesters.pages.dev/category-adventure-2.html)
- [CATEGORY STICKMAN175](https://skillplay.github.io/category-stickman175.html)
- [CATEGORY 1 PLAYER139](https://thelearnquesters.pages.dev/category-1-player139.html)
- [LOOPER](https://thelearnquester.web.app/looper.html)
- [DINO RANCH](https://themindskillplayplay.pages.dev/dino-ranch.html)
- [CAPYBARA COIN MASTER](https://themindskillplayplay.pages.dev/capybara-coin-master.html)
- [2 3 4 PLAYER GAMES](https://thelearnquester.web.app/2-3-4-player-games.html)
- [BLOCKS AND THATS IT](https://studyplayings.web.app/blocks-and-thats-it.html)
- [CATEGORY SIDE SCROLLING184](https://themindplay.github.io/category-side-scrolling184.html)
- [SORTING CANDY FACTORY](https://skillplay.github.io/sorting-candy-factory.html)
- [LAST UFO DEFENSE](https://themindplay.pages.dev/last-ufo-defense.html)
- [CATEGORY TOWER DEFENSE](https://thelearnquester.web.app/category-tower-defense.html)
- [HAWAII MATCH 6](https://learnquesters.pages.dev/hawaii-match-6.html)
- [STICKMAN PUNISHMENT](https://quizverses.pages.dev/stickman-punishment.html)
- [SQUID GAME MEMORY CARD MATCH](https://skillplay.github.io/squid-game-memory-card-match.html)
- [FAMILY PET DOG GAMES](https://iskillplay.web.app/family-pet-dog-games.html)
- [TURBO TRUCKS RACE](https://thelearnquester.web.app/turbo-trucks-race.html)
- [QUEENS ROYAL SUDOKU PUZZLE](https://themindplay.pages.dev/queens-royal-sudoku-puzzle.html)
- [SAUSAGE MAN SHOOTING ADVENTURE](https://quizverses.github.io/sausage-man-shooting-adventure.html)
- [CATEGORY BATTLE](https://skillplay.github.io/category-battle.html)
- [CATEGORY POOL](https://learnquesters.pages.dev/category-pool.html)
- [BUBBLE SHOOTER REMASTERED](https://quizverses-9d2f2.web.app/bubble-shooter-remastered.html)
- [SPACE CLEANER](https://studyplayings.web.app/space-cleaner.html)
- [SOLITAIRE FARM SEASONS 3](https://quizverses-9d2f2.web.app/solitaire-farm-seasons-3.html)
- [GROW A GARDEN 3D](https://iskillquest.pages.dev/grow-a-garden-3d.html)
- [MARBLE BLAST](https://studyplayings.web.app/marble-blast.html)
- [CATEGORY ROGUELIKE38](https://thelearnquesters.pages.dev/category-roguelike38.html)
- [COLOR 3D BUMP IT UP](https://quizverses-9d2f2.web.app/color-3d-bump-it-up.html)
- [ULTRAHERO VS MONSTERS ROYALE BATTLE](https://themindplays.pages.dev/ultrahero-vs-monsters-royale-battle.html)
- [ZUMBLE STORY](https://thelearnquester.web.app/zumble-story.html)
- [OBBY TOILET LINE](https://skillplay.github.io/obby-toilet-line.html)
- [CATEGORY ROGUELIKE GAMES](https://studyplayings.pages.dev/category-roguelike-games.html)
- [12 MINUTE ESCAPE](https://quizverses.github.io/12-minute-escape.html)
- [MINICRAFT WINTERBLOCK](https://quizverses.pages.dev/minicraft-winterblock.html)
- [BASE JUMP WINGSUIT FLYING](https://theskillquest.pages.dev/base-jump-wingsuit-flying.html)
- [OVERTIDE IO](https://studyquests.github.io/overtide-io.html)
- [SLENDER BOY ESCAPE ROBBIE](https://thequizzone.pages.dev/slender-boy-escape-robbie.html)
- [ISLAND EXPANDER](https://themindplays.pages.dev/island-expander.html)
- [JIGSAW M](https://quizverses.pages.dev/jigsaw-m.html)
- [HOPNOVA](https://themindplays.pages.dev/hopnova.html)
- [DINOSAUR RAMPAGE](https://studyquests.github.io/dinosaur-rampage.html)
- [POOL MERGE MANIA](https://iskillquest.pages.dev/pool-merge-mania.html)
- [CITY GAS STATION SIMULATOR](https://thequizzone.pages.dev/city-gas-station-simulator.html)
- [WOOLLOOP COLOR PUZZLE](https://skillplay.github.io/woolloop-color-puzzle.html)
